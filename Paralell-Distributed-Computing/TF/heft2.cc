...
float rankCalculate(int node)
{
  ...
  { // for every node
    ...
    {                              // that has an incoming edge from main node
      sub_rank = rankCalculate(i); // calculate its upper rank
      if (sub_rank + communication_cost_dag[node][i] > temp_max)
      {                                                        // if the upper rank is max among others
...
int jobInsertion(int search_end, vector<vector<bool>> &processor_state, int job_index, int processor_index, int search_start)
{
  ...
  for (int i = search_start; i < search_end; i++)
  {
    ...
    if (counter == t)
    { // as soon as a compatible gap is found, return its location
      return i;
    }
  }
...
void schedule(vector<int> rank_index_sorted)
{
  ...
  for (int i = 0; i < job_count; i++)
  {
    ...
    for (int j = 0; j < processor_count; j++)
    { // for each processor w.r.t each job
      for (int k = 0; k < jobs[ii].parents.size(); k++)
      {                                                    // for each parent node of the job calculate ready times
    ...
    for (int j = jobs[ii].st; j < jobs[ii].ft; j++)
    { // update the state of processor to reflect that processor was in use for the entire duration of job execution
